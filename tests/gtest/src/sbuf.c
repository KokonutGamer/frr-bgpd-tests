...
const char *sbuf_buf(struct sbuf *buf) { return buf->buf; }

void sbuf_free(struct sbuf *buf) {
  if (!buf->fixed) XFREE(MTYPE_TMP, buf->buf);
}
...
void sbuf_push(struct sbuf *buf, int indent, const char *format, ...) {
  ...
  if (!buf->fixed) {
    ...
    if (written1 >= 0 && written2 >= 0) {
      while (buf->pos + written1 + written2 >= new_size) new_size *= 2;
  ...
  if (written >= 0) buf->pos += written;
  if (buf->pos > buf->size) buf->pos = buf->size;
  ...
  if (buf->pos == buf->size) assert(!"Buffer filled up!");
}